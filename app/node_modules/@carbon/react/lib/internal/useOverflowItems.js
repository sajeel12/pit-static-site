...
const require_useResizeObserver = require('./useResizeObserver.js');
...
const useOverflowItems = (items, containerRef, offsetRef, maxItems, onChange) => {
	...
	const [maxWidth, setMaxWidth] = (0, react.useState)(0);
	const overflowItems = (0, react.useMemo)(() => Array.isArray(items) ? items : [], [items]);
	const handleResize = () => {
	...
	const hiddenItems = (0, react.useMemo)(() => {
		return overflowItems.slice(visibleItems.length);
	}, [overflowItems, visibleItems]);
	const previousHiddenItems = require_usePreviousValue.usePreviousValue(hiddenItems);
	(0, react.useEffect)(() => {

/**
 * Copyright IBM Corp. 2016, 2026
 *
 * This source code is licensed under the Apache-2.0 license found in the
 * LICENSE file in the root directory of this source tree.
 */

const require_runtime = require('../../_virtual/_rolldown/runtime.js');
const require_usePrefix = require('../../internal/usePrefix.js');
const require_SkeletonIcon = require('../SkeletonIcon/SkeletonIcon.js');
let classnames = require("classnames");
classnames = require_runtime.__toESM(classnames);
let react = require("react");
react = require_runtime.__toESM(react);
let prop_types = require("prop-types");
prop_types = require_runtime.__toESM(prop_types);
let react_jsx_runtime = require("react/jsx-runtime");

//#region src/components/AISkeleton/AISkeletonIcon.tsx
/**
* Copyright IBM Corp. 2016, 2024
*
* This source code is licensed under the Apache-2.0 license found in the
* LICENSE file in the root directory of this source tree.
*/
const AISkeletonIcon = ({ className, ...rest }) => {
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(require_SkeletonIcon.default, {
		className: (0, classnames.default)(className, { [`${require_usePrefix.usePrefix()}--skeleton__icon--ai`]: true }),
		...rest
	});
};
AISkeletonIcon.propTypes = {
	className: prop_types.default.string,
	style: prop_types.default.object
};

//#endregion
exports.default = AISkeletonIcon;
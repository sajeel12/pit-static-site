...
let flatpickr_dist_plugins_rangePlugin = require("flatpickr/dist/plugins/rangePlugin");
flatpickr_dist_plugins_rangePlugin = require_runtime.__toESM(flatpickr_dist_plugins_rangePlugin);
...
const rangePlugin = (config = {}) => {
	const factory = (0, flatpickr_dist_plugins_rangePlugin.default)(Object.assign({ position: "left" }, config));
	return (fp) => {
		...
		const init = () => {
			fp.setDate = (dates, triggerChange, format) => {
				...
				if (!triggerChange && Array.isArray(dates) && dates.length === 2) {
					...
					[inputFrom, typeof inputTo === "string" ? document.querySelector(inputTo) : inputTo].forEach((input, i) => {
						if (input && input instanceof HTMLInputElement) input.value = require_utils.isEmptyDateValue(dates[i]) ? "" : formatDate(new Date(dates[i]), fp.config.dateFormat);
					});
				}
			};
		};

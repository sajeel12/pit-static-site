/**
 * Copyright IBM Corp. 2016, 2026
 *
 * This source code is licensed under the Apache-2.0 license found in the
 * LICENSE file in the root directory of this source tree.
 */

const require_runtime = require('../../_virtual/_rolldown/runtime.js');
const require_usePrefix = require('../../internal/usePrefix.js');
let classnames = require("classnames");
classnames = require_runtime.__toESM(classnames);
let react = require("react");
react = require_runtime.__toESM(react);
let prop_types = require("prop-types");
prop_types = require_runtime.__toESM(prop_types);
let react_jsx_runtime = require("react/jsx-runtime");

//#region src/components/BadgeIndicator/index.tsx
/**
* Copyright IBM Corp. 2016, 2026
*
* This source code is licensed under the Apache-2.0 license found in the
* LICENSE file in the root directory of this source tree.
*/
const frFn = react.forwardRef;
const BadgeIndicator = frFn((props, ref) => {
	const { className: customClassName, count, ...rest } = props;
	const prefix = require_usePrefix.usePrefix();
	const classNames = (0, classnames.default)(`${prefix}--badge-indicator`, customClassName, { [`${prefix}--badge-indicator--count`]: count });
	const displayCount = count && count > 999 ? "999+" : count;
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
		className: classNames,
		ref,
		...rest,
		children: displayCount
	});
});
BadgeIndicator.propTypes = {
	className: prop_types.default.string,
	count: prop_types.default.number,
	id: prop_types.default.string
};

//#endregion
exports.default = BadgeIndicator;
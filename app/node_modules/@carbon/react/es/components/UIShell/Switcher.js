...
const Switcher = forwardRef((props, forwardRef) => {
	...
	const className = classNames(`${prefix}--switcher`, { [customClassName || ""]: !!customClassName });
	const handleSwitcherItemFocus = ({ currentIndex, direction }) => {
		const enabledIndices = Children.toArray(children).reduce((acc, child, i) => {
			if (isComponentElement(child, SwitcherItem) && Object.keys(child.props).length) acc.push(i);
			return acc;
		}, []);
	...
	const childrenWithProps = Children.toArray(children).map((child, index) => {
		if (isComponentElement(child, SwitcherItem)) return cloneElement(child, {
			handleSwitcherItemFocus,
			index,
			key: index,
			expanded
		});
		if (isComponentElement(child, SwitcherDivider)) return cloneElement(child, { key: index });
		return child;
	});

...
* LICENSE file in the root directory of this source tree.
*/
const HeaderPanel = React.forwardRef(({ children, className: customClassName, expanded, addFocusListeners = true, onHeaderPanelFocus = noopFn, href, ...rest }, ref) => {
	const prefix = usePrefix();
	const headerPanelReference = useRef(null);
	const headerPanelRef = useMergedRefs([headerPanelReference, ref]);
	const controlled = useRef(expanded !== void 0).current;
	...
	const className = classNames(`${prefix}--header-panel`, {
	...
	useWindowEvent("click", (event) => {
		...
		if (isComponentElement(children, Switcher) && !target.closest(`.${prefix}--header-panel--expanded`) && !target.closest(`.${prefix}--header__action`) && !headerPanelReference?.current?.classList.contains(`${prefix}--switcher`) && expanded) {
			setExpandedState(false);
			onHeaderPanelFocus();
		}
	});

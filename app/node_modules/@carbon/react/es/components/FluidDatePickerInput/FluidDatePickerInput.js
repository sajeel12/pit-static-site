/**
 * Copyright IBM Corp. 2016, 2026
 *
 * This source code is licensed under the Apache-2.0 license found in the
 * LICENSE file in the root directory of this source tree.
 */

import { FormContext } from "../FluidForm/FormContext.js";
import DatePickerInput_default from "../DatePickerInput/index.js";
import React, { forwardRef } from "react";
import { jsx } from "react/jsx-runtime";

//#region src/components/FluidDatePickerInput/FluidDatePickerInput.tsx
/**
* Copyright IBM Corp. 2022, 2025
*
* This source code is licensed under the Apache-2.0 license found in the
* LICENSE file in the root directory of this source tree.
*/
const FluidDatePickerInput = forwardRef((props, ref) => {
	return /* @__PURE__ */ jsx(FormContext.Provider, {
		value: { isFluid: true },
		children: /* @__PURE__ */ jsx(DatePickerInput_default, {
			ref,
			...props
		})
	});
});
FluidDatePickerInput.propTypes = DatePickerInput_default.propTypes;

//#endregion
export { FluidDatePickerInput as default };
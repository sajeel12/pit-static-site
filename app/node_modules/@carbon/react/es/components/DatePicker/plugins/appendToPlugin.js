...
//#region src/components/DatePicker/plugins/appendToPlugin.ts
	...
	*/
	const handlePreCalendarPosition = () => {
		Promise.resolve().then(() => {
			...
			if (!appendTo) throw new Error("[appendToPlugin] Missing `appendTo` element.");
			...
			if ((appendTo !== appendTo.ownerDocument.body || containerLeft !== 0 || containerTop !== 0) && appendTo.ownerDocument.defaultView?.getComputedStyle(appendTo).getPropertyValue("position") === "static") throw new Error("Floating menu container must not have `position: static`.");
